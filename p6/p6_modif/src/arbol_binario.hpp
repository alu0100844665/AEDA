...
void tree_t<T>::insert(const T& data) 
{
	...
	if (found != NULL)
	{
		//cout << "Elemento existente colision "<< found->get_data().getDni() <<found->get_data().getDniLetra()<<" "<<found->get_num_colisiones()<<endl;
		found->set_colision(1);
		//cout << "Elemento existente colision "<< found->get_data().getDni() <<found->get_data().getDniLetra()<<" "<<found->get_num_colisiones()<<endl;
		cerr << "Elemento existente colision " << endl;
	}
	else
	{
...
ostream& tree_t<T>::write(tree_node_t<T>* root, ostream& os,int n) const 
{
    ...
    while(!cola.empty())
    {
    	...
    	for(int c=0; c<i; c++)
		{
			...
			{
				//cout << "[" << aux->get_data().getDni() <<aux->get_data().getDniLetra()<<" "<<aux->get_num_colisiones()<< "]";
				//cout << "[" << aux->get_data().getDni() <<aux->get_data().getDniLetra()<<" "<<aux->get_data().getDniNombre()<< "]";
				cout << "[" << aux->get_data() << "]";
    	...
    	while(!Vacio(actual))
    	{
        	if(dat == actual->get_data())
        	{
            	if(is_sheet(actual))
            	{
                	...
                	if(actual->get_num_colisiones() == 1)
    				{
	                	delete actual;
	                	actual = NULL;
	            	  	return;
